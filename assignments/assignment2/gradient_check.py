...
def check_gradient(f, x, delta=1e-5, tol=1e-4, debug=False):
    """
    ...
    while not it.finished:
        ...
        if debug:
            print("DEBUG: Gradients at %s. Analytic: %2.5f, Numeric: %2.5f" % (
                ix, analytic_grad_at_ix, numeric_grad_at_ix))
            print(f'ration: {analytic_grad_at_ix / numeric_grad_at_ix}')
        else:
...
def check_model_gradient(model, X, y,
                         delta=1e-5, tol=1e-4, debug=False):
    """
    ...
    for param_key in params:
        ...
        if not check_gradient(helper_func, initial_w, delta, tol, debug):
            return False
